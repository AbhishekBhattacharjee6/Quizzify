...
class QuestionListAdapter(val frag:ActiveRoomQuestionList, var QuestionList:MutableList<ARQuestionListModel>):RecyclerView.Adapter<QuestionListAdapter.MyViewHolder>(){
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        val OriginalPosition=QuestionList.indexOf(QuestionList[position])
        ...
        val Data=Bundle()
        Data.putString("Question",QuestionInfo.Question)
        Data.putString("CA",QuestionInfo.CorrectAnswer)
        Data.putString("WA1",QuestionInfo.WrongAnswer1)
        Data.putString("WA2",QuestionInfo.WrongAnswer2)
        Data.putString("WA3",QuestionInfo.WrongAnswer2)
        Data.putString("QuizId",QuestionInfo.WrongAnswer3)
        holder.Question.text=QuestionInfo.Question
        holder.QuestionNumber.text=(position+1).toString()
        holder.EditBtn.setOnClickListener {
            frag.ReplaceFragfromAdapter(EditQuestionFragment(),Data)
        }
        holder.DelBtn.setOnClickListener {
            frag.DeleteQuiz(QuestionInfo.QuizSetID,QuestionList,OriginalPosition)
        }
        holder.OptionRV.layoutManager= LinearLayoutManager(frag.requireContext(), LinearLayoutManager.VERTICAL,false)
        val Options= listOf(OptionsModel(QuestionInfo.CorrectAnswer,true),
